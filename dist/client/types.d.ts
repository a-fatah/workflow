...
import type { GenericId, ObjectType, PropertyValidators, Validator } from "convex/values";
import type { WorkflowId, SignalHandle } from "../types.js";
import type { SignalDocument, Event, EventWorkflow } from "../component/schema.js";
...
export type SignalDefinition<T = any, M = any> = Validator<T> | {
    returns: Validator<T, any, any>;
    metadata?: Validator<M, any, any>;
};
export type SignalsDefinition = Record<string, SignalDefinition>;
export type ExtractReturns<SD extends SignalDefinition> = SD extends Validator<infer T> ? T : SD extends {
    returns: Validator<infer T, any, any>;
} ? T : never;
...
export interface WorkflowSignalHelpers<SignalsValidator extends SignalsDefinition = SignalsDefinition> {
    create<K extends keyof SignalsValidator>(name: K): Promise<SignalHandle<ExtractReturns<SignalsValidator[K]>, ExtractMetadata<SignalsValidator[K]>>>;
    create<Returns>(name: string, config: {
    ...
    awaitSignal: <Returns>(handle: SignalHandle<Returns>, options?: {
        timeoutMs?: number;
    }) => Promise<Returns>;
    all: <Handles extends Record<string, SignalHandle<any>>>(handles: Handles) => Promise<{
        [K in keyof Handles]: SignalValue<Handles[K]>;
    }>;
...
export type SignalValue<Handle extends SignalHandle<any>> = Handle extends SignalHandle<infer T> ? T : never;
...
export type WorkflowStep<SignalsValidator extends SignalsDefinition = SignalsDefinition> = {
     ...
     */
    pause<Mutation extends FunctionReference<"mutation", "internal">, Returns = unknown>(opts?: {
        /**
...
export type OpaqueIds<T> = T extends GenericId<infer _T> ? string : T extends WorkId ? string : T extends (infer U)[] ? OpaqueIds<U>[] : T extends object ? {
 ...
 */
export type EventDefinition<PayloadValidator extends PropertyValidators, Payload extends ObjectType<PayloadValidator> = ObjectType<PayloadValidator>> = {
    name: string;
 ...
 */
export type DefinedEvent<PayloadValidator extends PropertyValidators, Payload extends ObjectType<PayloadValidator> = ObjectType<PayloadValidator>> = {
    name: string;
    validator: PayloadValidator;
    _payload?: Payload;
    _topicId?: string;
 ...
 */
export type EventPayload<E extends DefinedEvent<any>> = E extends DefinedEvent<infer V extends PropertyValidators> ? ObjectType<V> : never;
/**
 * Type constraint for event handlers.
 * Ensures that the handler's args match the event payload structure.
 */
export type EventHandler<E extends DefinedEvent<any>> = E extends DefinedEvent<infer PayloadValidator extends PropertyValidators> ? FunctionReference<"mutation", "internal", ObjectType<PayloadValidator>, any> : never;
/**
