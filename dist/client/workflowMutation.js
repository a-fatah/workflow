...
export function workflowMutation(component, registered, defaultWorkpoolOptions) {
    ...
    return internalMutationGeneric({
        handler: async (ctx, args) => {
            ...
            const { workflow, logLevel, journalEntries, ok } = await ctx.runQuery(component.journal.load, { workflowId });
            ...
            if (workflow.generationNumber !== generationNumber) {
                console.error(`Invalid generation number: ${generationNumber} running workflow ${workflow.name} (${workflowId})`);
                return;
            }
            ...
            const step = new StepContext(workflowId, channel, component, ctx, generationNumber, registered.signals);
            const executor = new StepExecutor(workflowId, generationNumber, ctx, component, journalEntries, channel, Date.now(), workpoolOptions);
                ...
                try {
                    ...
                    if (registered.returns) {
                        try {
                            ...
                            console.error("Workflow handler returned invalid return value: ", message);
                            runResult = {
                                kind: "failed",
