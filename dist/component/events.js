...
import { eventDocument, eventWorkflowDocument, topicDocument, } from "./schema.js";
...
export const markWorkflowCompleted = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const eventId = args.context?.eventId;
        if (!eventId) {
        ...
        const status = args.result.kind === "success"
            ? "completed"
 ...
 */
async function updateEventStatusIfComplete(ctx, eventId) {
    const console = await getDefaultLogger(ctx);
    const eventWorkflows = await ctx.db
        .query("eventWorkflows")
    ...
    const allComplete = eventWorkflows.every((ew) => ew.status === "completed" ||
        ew.status === "failed" ||
        ew.status === "canceled");
    if (!allComplete) {
        return;
...
export const replayEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.workflowHandle) {
            registrations = registrations.filter((r) => r.workflowHandle === args.workflowHandle);
            if (registrations.length === 0) {
...
export const listPendingEvents = query({
    ...
    handler: async (ctx, args) => {
        ...
        const tableQuery = ctx.db.query("events");
...
export const listEventsByTopic = query({
    args: {
        topicId: v.id("topics"),
        status: v.optional(v.union(v.literal("pending"), v.literal("dispatching"), v.literal("completed"), v.literal("failed"))),
        limit: v.optional(v.number()),
    },
    returns: v.object({
        events: v.array(eventDocument),
