import { vResultValidator, vRetryBehavior, vWorkIdValidator, Workpool, } from "@convex-dev/workpool";
...
export async function getWorkpool(ctx, opts) {
    // nit: can fetch config only if necessary
    const config = await ctx.db.query("config").first();
    const logLevel = opts?.logLevel ?? config?.logLevel;
    const maxParallelism = opts?.maxParallelism ?? config?.maxParallelism ?? DEFAULT_MAX_PARALLELISM;
...
export const onComplete = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const stepId = "stepId" in args.context
        ...
        if (!validate(onCompleteContext, args.context, { allowUnknownFields: true })) {
            const error = `Invalid onComplete context for workId ${args.workId}` +
        ...
        if (workflow.generationNumber !== generationNumber) {
            console.error(`Workflow: ${workflowId} already has generation number ${workflow.generationNumber} when completing ${stepId}`);
            return;
        }
        if (!journalEntry.step.inProgress) {
            console.error(`Step finished but journal entry not in progress: ${stepId} status: ${journalEntry.step.runResult?.kind ?? "pending"}`);
            return;
        }
        if (journalEntry.step.type === "pause" &&
        ...
        if (workflow.runResult !== undefined) {
            if (workflow.runResult.kind !== "canceled") {
                console.error(`Workflow: ${workflowId} already ${workflow.runResult.kind} when completing ${stepId} with status ${args.result.kind}`);
            }
            return;
        }
        const workpool = await getWorkpool(ctx, args.context.workpoolOptions);
        await workpool.enqueueMutation(ctx, workflow.workflowHandle, { workflowId: workflow._id, generationNumber }, {
            name: workflow.name,
...
export const handlerOnComplete = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!validate(handlerOnCompleteContext, args.context)) {
            ...
            const workflowId = ctx.db.normalizeId("workflows", args.context.workflowId);
            await ctx.db.insert("onCompleteFailures", args);
            if (!workflowId) {
            ...
            await completeHandler(ctx, {
                ...
                runResult: {
                    kind: "failed",
                    error: "Invalid handlerOnComplete context: " +
                        JSON.stringify(args.context),
                },
            }).catch((error) => {
